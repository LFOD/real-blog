...
I knew I could do better. I remembered that I had read in this charming [AWK guide by Bruce Barnett](http://www.grymoire.com/Unix/Awk.html) about a cool feature in AWK called ["associative arrays'](http://www.grymoire.com/Unix/Awk.html#uh-22). These are essentially a key value stores in AWK that for some reason had been given a different name and thus I never thought too much about. I realized that I could use these associative arrays to perform the union between my SNP -> bin table and my raw data without using Spark. 
...
After this bash script had finished I have a bunch of `.rds` files sitting in S3 benefiting from the great compression techniques and also types built in that `.rds` offers.  
...
It turns out S3 treats the path to a given file as a simple key in what can be thought of as a hash table, or a document-based database. Think of a "bucket" as a table and every file is an entry. Because of this folders in S3 are just a cosmetic thing and S3 doesn't actually care about the `/` character. 
...
In addition, since a common workflow is scanning SNP-by-SNP along a region, caching the bin made it very likely that the next requested SNP would already be on the machine and thus could be immediately grabbed. 
