...
__Just show me the sollution!__ If you just want to see how I ended up solving the task jump to the section <a href='#getting-more-creative'>Getting More Creative</a>, but I honestly think the failures are more interesting/valuable. 
...
![](/using_awk_and_r_to_parse_25tb/uneven_snps.png)
...
Given that all my Spark failures were due to the data being shuffled around the cluster because it was starting all mixed up, I figured I could attempt to help Spark out by preprocessing. I decided to try and split the raw text data on the chromosome column, that way I would be able to provide Spark with somewhat 'pre-partitioned' data. 
...
Once I ran the splitting using the new GNU parallel workflow it was great, but I was still getting some bottlenecking caused by downloading the S3 objects to disk being a little bit slow and not fully parallelized. I did a few things to fix this. 
...
4. Swapped `gzip` to [`pigz`](https://linux.die.net/man/1/pigz), which is a parallel gzip tool that does some clever things to parallelize an inherently unparallizable task of decompressing gziped files. (This helped the least.)
