...
There is also a stand-alone htmlwidget available, however, at this point it doesn't do much other than show you an image and give you a useless 'Take Photo!' button. 

```{r}
shinysense::viewr_widget(
  width = '200px', 
  height = '200px'
)
```

`r margin_note("In the future I am interested in looking at the potential to port models from Keras directly to javascript using [tensorflow.js](https://js.tensorflow.org/) which would allow you to train a model in R and run it in a static document like an RMarkdown report.")`

The widget is used internally for the function `shinyviewr`, but also helps to demo the function without needing a server to run a shiny app!
