...
If you require a little more convincing, let's introduce a third study that falls in the middle, **Study C**. This study has 100 participants and an effect size of -1. `r tufte::margin_note("How am I picking these effect sizes? We know the p-value is a function of the effect size and standard error, so I've chosen each effect size such that the ratio of the effect size to standard error would be equal. For example, in the first case, this is $\\frac{-2.5}{sd/\\sqrt(16)}$, and in the second case, this is  $\\frac{-0.1}{sd/\\sqrt(10000)}$, both of which equal 10/sd.")`
