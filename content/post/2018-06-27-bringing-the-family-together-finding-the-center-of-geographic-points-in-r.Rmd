...
```{r}
unweighted_avg <- geographic_average(
  lon = df$lon,
  lat = df$lat)
weighted_avg <- geographic_average(
  lon = df$lon,
  lat = df$lat,
  weight = df$number)

unweighted_mid <- geographic_midpoint(
  lon = df$lon,
  lat = df$lat)
weighted_mid <- geographic_midpoint(
  lon = df$lon, 
  lat = df$lat,
  weight = df$number)

unweighted_centroid <- geosphere::centroid(
  df[, c("lon", "lat")])
```
...
unweighted_avg_nospain <- geographic_average(
  lon = df_nospain$lon,
  lat = df_nospain$lat)
weighted_avg_nospain <- geographic_average(
  lon = df_nospain$lon,
  lat = df_nospain$lat,
  weight = df_nospain$number)

unweighted_mid_nospain <- geographic_midpoint(
  lon = df_nospain$lon, 
  lat = df_nospain$lat)
weighted_mid_nospain <- geographic_midpoint(
  lon = df_nospain$lon,
  lat = df_nospain$lat,
  weight = df_nospain$number)

unweighted_centroid_nospain <- geosphere::centroid(
...
```{r}
cols <- paletteer::paletteer_d("LaCroixColoR", "Pamplemousse", n = 12, type = "continuous")
```
...
leaflet() %>%
  ...
  addCircleMarkers(lng = df$lon,
                   ...
                   label = df$city,
                   color = cols[1]) %>%
  addCircleMarkers(lng = unweighted_avg$lon,
                   lat = unweighted_avg$lat,
                   color = cols[2],
                   label = "Unweighted Average") %>%
  addCircleMarkers(lng = unweighted_avg_nospain$lon, 
                   lat = unweighted_avg_nospain$lat, 
                   color = cols[3], 
                   label = "Unweighted Average (No Spain)") %>%
  addCircleMarkers(lng = weighted_avg$lon,
                   lat = weighted_avg$lat,
                   color = cols[4], 
                   label = "Weighted Average") %>%
  addCircleMarkers(lng = weighted_avg_nospain$lon,
                   lat = weighted_avg_nospain$lat, 
                   color = cols[5], 
                   label = "Weighted Average (No Spain)") %>%
  addCircleMarkers(lng = unweighted_mid$lon,
                   lat = unweighted_mid$lat, 
                   color = cols[6], 
                   label = "Unweighted Midpoint") %>%
  addCircleMarkers(lng = unweighted_mid_nospain$lon,
                   lat = unweighted_mid_nospain$lat, 
                   color = cols[8], 
                   label = "Unweighted Midpoint (No Spain)")%>%
  addCircleMarkers(lng = weighted_mid$lon,
                   lat = weighted_mid$lat,
                   color = cols[9], 
                   label = "Weighted Midpoint") %>%
  addCircleMarkers(lng = weighted_mid_nospain$lon, 
                   lat = weighted_mid_nospain$lat, 
                   color = cols[10], 
                   label = "Weighted Midpoint (No Spain)") %>%
  addCircleMarkers(lng = unweighted_centroid[1],
                   lat = unweighted_centroid[2], 
                   color = cols[11], 
                   label = "Centroid") %>%
  addCircleMarkers(lng = unweighted_centroid_nospain[1], 
                   lat = unweighted_centroid_nospain[2], 
                   color = cols[12],
                   label = "Centroid (No Spain)")
```
